...
### ArchivedDay
```
id: UUID                   # surrogate PK - identifies the row for DELETE /archive/:id
dayOfWeek: number (0-6)
month: number (0-11)
day: number (1-31)
yr: number
...
row; it's never silently duplicated.
...
**Re-archiving an already-archived date:** the `(user_id, yr, mon, day)`
unique constraint means a second `POST /archive/:dayOfWeek` for the same
calendar date conflicts (`409 ARCHIVE_ALREADY_EXISTS`) unless the request
also passes `?onConflict=merge` or `?onConflict=overwrite`:

- `merge` - appends the newly-archived events onto the existing row's
  `events`/`count`/`sum` (nothing from the earlier archive is lost)
- `overwrite` - replaces the existing row's snapshot entirely

The client always confirms first: archiving prompts the normal confirm
dialog, and only on a conflict response does it show a second dialog letting
the user pick merge or overwrite (never silently applies either).
