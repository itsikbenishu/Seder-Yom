...
import {
  ...
  type ArchiveDayAllDayEventDetailProps,
} from "../../../types/archiveDay";
import type { AllDayCalendarEvent } from "../../../types/calendarEvent";
import { AttachmentsSection } from "../day/AttachmentsSection";

function isLocalEvent(event: ArchiveDayAllDayEvent): event is AllDayCalendarEvent {
  return !isGoogleCalendarEvent(event);
}

export function ArchiveDayAllDayEventDetail({ event, onClose }: ArchiveDayAllDayEventDetailProps) {
  const { t } = useTranslation();
  const isGoogle = isGoogleCalendarEvent(event);

  return (
    <Modal open title={event.title} onClose={onClose} className="max-w-[440px]">
